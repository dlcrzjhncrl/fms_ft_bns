...
Set Batch Admin Environment Parameter
    [Documentation]    This keyword will set value to the chosen environment parameter in batch admin notebook
    ...    @author: dahijara    25MAR2021    - Initial create
    [Arguments]    ${sZone}    ${sEnv_Column_Name}    ${sEnv_Column_Value}

    ### Pre-processing Keyword ###
    ${Zone}    Acquire Argument Value    ${sZone}
    ${Env_Column_Name}    Acquire Argument Value    ${sEnv_Column_Name}
    ${Env_Column_Value}    Acquire Argument Value    ${sEnv_Column_Value}

    mx LoanIQ activate window    ${LIQ_Batch_Admin_Window}
    mx LoanIQ select    ${LIQ_Batch_Admin_Options_Environment}
    mx LoanIQ activate window    ${LIQ_Environment_Parameter_Window}
    Mx LoanIQ Select String    ${LIQ_Environment_Parameter_JavaTree}    ${Zone}\t${Env_Column_Name}    
    Mx Press Combination    KEY.ENTER
    mx LoanIQ activate window    ${LIQ_Environment_Param_Window}
    ${Env_Column_Value}    Run Keyword If    'Date' in '${Env_Column_Name}'    Convert Date Value per Zone    ${Env_Column_Value}    ${Zone}
    ...    ELSE    Set Variable    ${Env_Column_Value}
    mx LoanIQ enter    ${LIQ_Environment_Param_Value}    ${Env_Column_Value}
    mx LoanIQ click    ${LIQ_Environment_OK_Button}
    mx LoanIQ close window    ${LIQ_Environment_Parameter_Window}

Convert Date Value per Zone
    [Documentation]    This keyword will convert date format for Batch admin environment parameter per Zone
    ...    @author: dahijara    25MAR2021    - Initial create
    [Arguments]    ${sDate}    ${sZone}    ${sRunTimeVar_FormattedDateValue}=None
    ...
    ${Month}    Run Keyword If    '${Month}'=='Jan'    Set Variable    01
    ...    ELSE IF    '${Month}'=='Feb'    Set Variable    02
    ...    ELSE IF    '${Month}'=='Mar'    Set Variable    03
    ...    ELSE IF    '${Month}'=='Apr'    Set Variable    04
    ...    ELSE IF    '${Month}'=='May'    Set Variable    05
    ...    ELSE IF    '${Month}'=='Jun'    Set Variable    06
    ...    ELSE IF    '${Month}'=='Jul'    Set Variable    07
    ...    ELSE IF    '${Month}'=='Aug'    Set Variable    08
    ...    ELSE IF    '${Month}'=='Sep'    Set Variable    09
    ...    ELSE IF    '${Month}'=='Oct'    Set Variable    10
    ...    ELSE IF    '${Month}'=='Nov'    Set Variable    11
    ...    ELSE IF    '${Month}'=='Dec'    Set Variable    12
    ...    ELSE    Fail    Invalid Month Value - ${Month}.
    Log    ${month}

    ${FormattedDateValue}    Run Keyword If    '${Zone}'=='ZONE1'    Set Variable    ${Month}-${Day}-${Year}
    ...    ELSE IF    '${Zone}'=='ZONE2'    Set Variable    ${Month}-${Day}-${Year}
    ...    ELSE IF    '${Zone}'=='ZONE3'    Set Variable    ${Month}-${Day}-${Year}
    ...    ELSE IF    '${Zone}'=='ZONE4'    Set Variable    ${Month}-${Day}-${Year}

    Log    Zone: ${Zone} | Date: ${FormattedDateValue}

    ### Keyword Post-processing ###
    Save Values of Runtime Execution on Excel File    ${sRunTimeVar_FormattedDateValue}    ${FormattedDateValue}
    [Return]    ${FormattedDateValue}

Validate Current Business Date
    [Documentation]    This keyword will validate if the UI business date is equal to the scheduled target business date.
    ...    @author: dahijara    25MAR2021    - Initial create
    [Arguments]    ${sUIDate}    ${sExpectedDate}

    ### GetRuntime Keyword Pre-processing ###
    ${UIDate}    Acquire Argument Value    ${sUIDate}
    ${ExpectedDate}    Acquire Argument Value    ${sExpectedDate}
    
    Compare Two Strings    ${UIDate}    ${ExpectedDate}

Validate if Next Business Date Needs Update
    [Documentation]    This keyword is used to check if LIQ next business date needs to be updated with new target business date.
    ...    @author: dahijara    26MAR2021    - Initial create
    [Arguments]    ${sZone}    ${sEnv_Next_Business_Date_ColName}    ${sEnv_Next_Business_Date_ColValue}

    ### GetRuntime Keyword Pre-processing ###
    ${Zone}    Acquire Argument Value    ${sZone}
    ${Env_Next_Business_Date_ColName}    Acquire Argument Value    ${sEnv_Next_Business_Date_ColName}
    ${Env_Next_Business_Date_ColValue}    Acquire Argument Value    ${sEnv_Next_Business_Date_ColValue}

    Mx LoanIQ activate window    ${LIQ_Batch_Admin_Window}
    ${Zone_Curr_Date}    Mx LoanIQ Store TableCell To Clipboard    ${LIQ_Batch_Admin_TimeZone_JavaTree}    ${Zone}%Next%Zone_Curr_Date

    ${isMatched}    Run Keyword And Return Status    Should Be Equal    ${Zone_Curr_Date}    ${sEnv_Next_Business_Date_ColValue}
    Run Keyword If    ${isMatched}==${False}    Set Batch Admin Environment Parameter    ${sZone}    ${sEnv_Next_Business_Date_ColName}    ${sEnv_Next_Business_Date_ColValue}
    ...    ELSE    Log    Dates are equal, no updates needed. Proceed on adding schedule.      
